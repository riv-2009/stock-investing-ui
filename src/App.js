...
function App() {
	...
	useEffect(() => {
		if ((stockData && Action == "quit") || day == 8) {
			...
			setDayMsg(`Bank account balance: $${balance.toLocaleString("en-US")}`);
			...
			setMessage(
				`You can buy ${Math.trunc(shares)} shares, how many would you like?`
			);
	...
	return (
		<div className="container m-4">
			...
			{stockData !== "" && Action !== "quit" && (
				<>
					<InvestmentAccount bal={investmentBal} shares={sharesPurchased} />
					<BankAccount bal={bal} />
				</>
			)}
			{stockData !== "" && Action == "" && (
